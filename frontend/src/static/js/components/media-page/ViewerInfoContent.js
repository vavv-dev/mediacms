    ...
    <div className="media-author-banner">
      ...
      <div>
        ...
        {PageStore.get('config-media-item').displayPublishDate && props.published ? (
          <span className="author-banner-date">{i18next.t('Published on')} {publishedOnDate(new Date(props.published))}</span>
        ) : null}
      </div>
    </div>
  );
...
export default function ViewerInfoContent(props) {
  ...
  function onMediaDelete(mediaId) {
    ...
    setTimeout(function () {
      PageActions.addNotification(i18next.t('Media removed. Redirecting...'), 'mediaDelete');
      setTimeout(function () {
  ...
  function onMediaDeleteFail(mediaId) {
    // FIXME: Without delay creates conflict [ Uncaught Error: Dispatch.dispatch(...): Cannot dispatch in the middle of a dispatch. ].
    setTimeout(function () {
      PageActions.addNotification(i18next.t('Media removal failed'), 'mediaDeleteFail');
    }, 100);
        ...
        <div className="media-content-banner-inner">
          ...
          {hasSummary ? (
            <button className="load-more" onClick={onClickLoadMore}>
              {i18next.t(isContentVisible ? 'SHOW LESS' : 'SHOW MORE')}
            </button>
          ) : null}
          {tagsContent.length ? (
            <MediaMetaField value={tagsContent} title={i18next.t(1 < tagsContent.length ? 'Tags' : 'Tag')} id="tags" />
          ) : null}
          {categoriesContent.length ? (
            <MediaMetaField
              value={categoriesContent}
              title={i18next.t(1 < categoriesContent.length ? 'Categories' : 'Category')}
              id="categories"
            />
          ) : null}
          ...
          {userCan.editMedia || userCan.editSubtitle || userCan.deleteMedia ? (
            <div className="media-author-actions">
              ...
              <PopupTrigger contentRef={popupContentRef}>
                <button className="remove-media">{i18next.t('DELETE MEDIA')}</button>
              </PopupTrigger>

              <PopupContent contentRef={popupContentRef}>
                <PopupMain>
                  <div className="popup-message">
                    <span className="popup-message-title">{i18next.t('Media removal')}</span>
                    <span className="popup-message-main">{i18next.t("You're willing to remove media permanently?")}</span>
                  </div>
                  <hr />
